  ...
  function bubble(msg) {
    ...
    if (msg.sources && msg.sources.length) {
      ...
      msg.sources.slice(0, 4).forEach(function (x) {
        ...
        i.className = "go-srci";
        i.innerHTML =
          '<div class="go-srcn">' + esc(pretty(x.source)) + "</div>" +
          (x.snippet ? '<div class="go-srcs">' + esc(x.snippet) + "</div>" : "");
        s.appendChild(i);
      });
      b.appendChild(s);
    }
  ...
  function ask(q, opts) {
    ...
    fetch(cfg.api + "/query", {
      ...
      .then(function (d) {
        ...
        if (!opts.skipFaq && d.clarify && d.suggestions && d.suggestions.length) {
          say(d.answer || "Could you clarify?");
          chips(
            d.suggestions
              .map(function (sug) {
                return { label: sug, style: "q", onClick: function () { ask(sug); } };
              })
              .concat([{ label: "No \u2014 search the documentation instead", style: "alt",
        ...
        say(d.answer || "No answer returned.", {
          ...
          flagged: !!d.flagged,
          badge: d.from_faq
            ? "Curated answer" + (d.faq_matched_question ? " \u2014 matched: \u201c" + d.faq_matched_question + "\u201d" : "")
            : null,
        });
      })
      .catch(function (e) {
