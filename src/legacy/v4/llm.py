...
def _truncate_after_refusal(text: str) -> str:
    """
    Small local models sometimes emit a correct refusal sentence and then
    continue rambling into unrelated content (hypothetical scenarios,
    riddles, etc). If the refusal phrase appears anywhere in the output,
    cut everything after the sentence containing it.
    """
    lower = text.lower()
    idx = lower.find(REFUSAL_PHRASE)
    if idx == -1:
        return text

    # Find the end of the sentence containing the refusal phrase
    end = idx + len(REFUSAL_PHRASE)
...
def _call_ollama(
    ...
    if num_predict is None:
        # Reduced from 200 → 160 for mistral: cuts ~20% off long-tail latency
        # (query 5/6/12 were 60-113s) without truncating real list answers
    ...
    try:
        ...
        text = _truncate_after_refusal(text)
