...
def admin_faq_autogenerate(payload: FaqAutoReq,
    ...
    from db import get_collection
    col = get_collection()
    got = col.get(where={"source": payload.source}, include=["documents"])
    docs = [d for d in (got.get("documents") or []) if d and d.strip()]
    if not docs:
        raise HTTPException(status_code=404,
                            detail="No content found for that document")

    # A sample rather than the whole document: enough to characterise what
    # it covers, small enough to stay well inside the context window.
    sample = "\n\n".join(docs[:12])[:6000]
    n = max(3, min(payload.count, 15))
    prompt = (
        f"Below is an extract from a product support document.\n\n"
        f"Write {n} questions a customer would realistically ask that this "
        f"extract answers, each with a short factual answer taken ONLY from "
        f"the extract.\n\n"
        f"Return one per line in exactly this format, with no numbering, "
        f"no preamble and no other text:\n"
        f"QUESTION :: ANSWER\n\n"
        f"---\n{sample}"
    )
    ...
    try:
        ...
        result = generate_with_fallback("fast", prompt)
    except Exception as e:
        logger.exception("FAQ autogenerate failed")
        raise HTTPException(status_code=503,
                            detail=f"Could not reach the language model: {e}")
    ...
    for line in text.splitlines():
        ...
        q, a = q.strip().strip('"'), a.strip()
        if len(q.split()) >= 3 and a:
            pairs.append({"question": q if q.endswith("?") else q + "?",
                          "answer": a})
    if not pairs:
        raise HTTPException(
            status_code=422,
            detail="The model returned nothing usable. Try again, or add "
                   "questions by hand.")

    merged = faq_store.merge_questions(payload.source,
                                       payload.product or payload.category,
                                       pairs[:n], category=payload.category)
    return {**merged, "source": payload.source, "generated": len(pairs)}
