...
LIMITS = {
    "anonymous": int(os.getenv("QUOTA_ANON", "15")),
    "member": int(os.getenv("QUOTA_MEMBER", "30")),
    "staff": int(os.getenv("QUOTA_STAFF", "500")),
}

# Per-IP ceiling for anonymous traffic, independent of visitor id. Set
# generously enough for a shared office NAT but low enough to stop a
# scripted drain.
ANON_IP_LIMIT = int(os.getenv("QUOTA_ANON_IP", "60"))
...
def resolve_effort(requested: str | None, tier: str) -> tuple[str, dict]:
    ...
    """
    level = (requested or "standard").strip().lower()
    spec = EFFORT.get(level)
    if not spec:
        return "standard", EFFORT["standard"]
...
def status(caller: dict) -> dict:
    ...
    return {
        ...
        "remaining": max(0, limit - used),
        "reset_at": win + WINDOW_SECONDS,
        "deep_available": _TIER_RANK[caller["tier"]] >= _TIER_RANK["member"],
