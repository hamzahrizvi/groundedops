...
Two implementations failed in production:
  ...
  An early attempt used lexical Jaccard @0.6, which — only fired on near-verbatim repeats, so
        the FAQ was effectively dead.
  A later attempt used bi-encoder cosine plus a cross-encoder verifier,
        which served
...
#
# v12.0: raised from 0.45/0.34. Those were set loose on the reasoning that
...
def _norm_key(k: str | None) -> str:
    ...
    v12.0: matching used to be exact string equality, so a catalog key of
...
def merge_questions(source: str, products: str, qa_pairs: list[dict],
                    category: str = "") -> dict:
    """NON-DESTRUCTIVE generate (v12.0).
