...
from catalog import product_for_source
...
def ingest_file(content: bytes, filename: str,
    ...
    try:
        ...
        if q_docs:
            ...
            # v10.4: record the generated questions (+ their source chunk as
            # the default answer) to the FAQ store so they can be shown and
            # edited per product on the FAQ page.
            try:
                import faq_store
                faq_store.record_questions(
                    filename, _prod_tag,
                    [{"question": q, "answer": m.get("parent_text", "")}
                     for q, m in zip(q_docs, q_metas)])
            except Exception as e:
                logger.warning(f"FAQ store write failed (non-fatal): {e}")
