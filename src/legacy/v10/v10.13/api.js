...
export const api = {
  ...
  // v10.13: generate FAQ IN THE BROWSER using the key already stored
  // client-side (no backend key needed). Calls the chosen provider's API
  // directly, returns a list of question strings.
  generateFaqQuestions: async (provider, contextText, n = 6) => {
    const prompt =
      `You write FAQ questions for a product support chatbot. Based on the ` +
      `documentation excerpt below, write ${n} distinct questions a customer ` +
      `is likely to ask that this material answers. Return ONLY the questions, ` +
    ...
    if (provider === "openai") {
      ...
      return splitLines(d.choices?.[0]?.message?.content || "");
    }
    if (provider === "anthropic") {
      ...
      const r = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({ model: "claude-3-5-haiku-latest", max_tokens: 500,
          messages: [{ role: "user", content: prompt }] }),
      });
      if (!r.ok) throw new Error(`Claude: ${r.status}`);
      const d = await r.json();
      return splitLines(d.content?.[0]?.text || "");
    }
    // default: deepseek
    const key = getKey();
