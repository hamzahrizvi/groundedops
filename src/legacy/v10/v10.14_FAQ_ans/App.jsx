...
import { SendIcon, RocketIcon, RunnerIcon, HelpIcon } from "./icons.jsx";
...
export default function App() {
  ...
  // v2.1: product scoping. `product` = active chat's product key;
  // `pickProduct` shows the pre-chat product picker.
  // v10.3: category -> product two-step scoping.
  ...
  useEffect(() => {
    api.ingestVersion().then((v) => setReingest(!!v.reingest_recommended)).catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setChats((prev) => {
      ...
      const entry = { id: sessionId, title, messages, updated: Date.now(), category, categoryName, product, productName };
  ...
  // v10.14: clicking a starter chip asks that question (was referenced but
  // undefined — chips did nothing on click).
  ...
  const newChat = async () => {
    // v10.3: choose a CATEGORY first, then optionally a product.
  ...
  const startChat = (cat, prod) => {
    setCategory(cat ? cat.key : null);
    setCategoryName(cat ? cat.name : null);
    setProduct(prod ? prod.key : null);
    setProductName(prod ? prod.name : null);
    setSessionId(uuid());
    setMessages([]);
    setSelected(null);
    setIntroDone(true);
    setPickStep(null);
    setPickCat(null);
    // v10.6: load FAQ questions for this scope as conversation starters.
    const scopeKey = prod ? prod.key : (cat ? cat.key : null);
    if (scopeKey) {
      api.faq(scopeKey)
        .then((r) => setSuggestions((r.faq || []).slice(0, 5).map((f) => f.question)))
  ...
  const openChat = (chat) => {
    restoringRef.current = true;
    setProduct(chat.product || null);
    setProductName(chat.productName || null);
  ...
  return (
    <div className="app">
        ...
        <div className="stage" ref={stageRef}>
          {showSplash ? (
            ...
            <div className="stage-inner">
              ...
              {view === "faq" && (
                <FaqPage catalog={catalog} onClose={() => setView("chat")} />
              )}
              {view === "chats" && (
                <ChatsPage
                  chats={chats}
              ...
              {(categoryName || productName) && view === "chat" && (
                <div className="scope-bar">
                  <div className="product-badge" title="This chat is scoped">
                    ...
                    {suggestions.length > 0 && (
                      <div className="suggestions">
                        <p className="suggestions-label">
                          {productName ? `Common questions about ${productName}:` : "Common questions:"}
                        </p>
                        {suggestions.map((q, i) => (
      ...
      {view === "chat" && (categoryName || productName) && (
        <div className={`faq-dock ${faqPanelOpen ? "open" : ""}`}>
          <button className="faq-handle" onClick={() => setFaqPanelOpen((v) => !v)}
                  title="Frequently asked questions" aria-label="FAQ">
            <HelpIcon width={22} height={22} />
          </button>
          {faqPanelOpen && (
            <ChatFaqDrawer
              scopeKey={product || category}
              scopeIsCategory={!product && !!category}
              catalog={catalog}
              onAsk={(q) => { setFaqPanelOpen(false); runQuery(q); }}
      ...
      {pickStep === "category" && (
        <div className="overlay" onClick={() => setPickStep(null)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-head"><h2>Choose a category</h2></div>
            <p className="hint" style={{ marginTop: 0 }}>
              Pick the product family this question is about.
            </p>
            <div className="product-grid">
              {catalog.categories.map((c) => (
                <button key={c.key} className="product-btn"
      ...
      {pickStep === "product" && pickCat && (
        <div className="overlay" onClick={() => setPickStep("category")}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-head"><h2>{pickCat.name} — narrow it down?</h2></div>
            <p className="hint" style={{ marginTop: 0 }}>
              Pick a specific product, or search the whole {pickCat.name} category.
            </p>
            <div className="product-grid">
              <button className="product-btn product-all"
                      disabled={!(pickCat.doc_count > 0)}
                      title={pickCat.doc_count > 0 ? "" : "No documents in this category yet"}
                      onClick={() => pickCat.doc_count > 0 && startChat(pickCat, null)}>
                All of {pickCat.name}
                {!(pickCat.doc_count > 0) && <span className="no-docs"> · no documents</span>}
              </button>
              {pickCat.products.map((p) => (
                <button key={p.key} className="product-btn"
                        disabled={!(p.doc_count > 0)}
                        title={p.doc_count > 0 ? "" : "No documents attached to this product yet"}
                        onClick={() => p.doc_count > 0 && startChat(pickCat, p)}>
                  {p.name}
                  {!(p.doc_count > 0) && <span className="no-docs"> · no documents</span>}
                </button>
              ))}
            </div>
            <div className="dialog-actions">
              <button className="btn" onClick={() => setPickStep("category")}>← Back</button>
            </div>
          </div>
        </div>
      )}
