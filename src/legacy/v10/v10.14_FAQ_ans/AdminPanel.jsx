...
export default function AdminPanel({ onClose, onCatalogChanged }) {
  ...
  const [faqModel, setFaqModel] = useState("");        // "" = provider default
  const [faqDepth, setFaqDepth] = useState(10);         // basic/thorough/deep/custom
  const [genBusy, setGenBusy] = useState(false);

  // curated model lists per provider (v10.14)
  const MODELS = {
    deepseek: [["", "deepseek-chat (default)"], ["deepseek-reasoner", "deepseek-reasoner (advanced)"]],
    openai: [["", "gpt-4o-mini (default)"], ["gpt-4o", "gpt-4o (advanced)"], ["gpt-4.1", "gpt-4.1"]],
    anthropic: [["", "claude-3-5-haiku (default)"], ["claude-sonnet-4-5", "claude-sonnet-4.5 (advanced)"]],
  };
  ...
  const generateFaq = async () => {
    ...
    try {
      ...
      const qa = await api.generateFaqQA(faqGenProvider, sample, faqDepth, faqModel || null);
      if (!qa.length) throw new Error("The model returned no Q&A — try again, a different model, or check the key.");
      await api.storeFaq(pw, faqSource, meta.product || "", meta.category || "", qa);
        ...
        {tab === "faq" && (
          <>
            ...
            {faqSource && (
              <>
                <div className="admin-row">
                  <label style={{ alignSelf: "center", fontSize: 13, opacity: 0.8 }}>Provider:</label>
                  <select value={faqGenProvider}
                          onChange={(e) => { setFaqGenProvider(e.target.value); setFaqModel(""); }}>
                    <option value="deepseek">DeepSeek</option>
                    <option value="openai">OpenAI</option>
                    <option value="anthropic">Claude</option>
                  </select>
                  <select value={faqModel} onChange={(e) => setFaqModel(e.target.value)}>
                    {(MODELS[faqGenProvider] || []).map(([v, label]) => (
                      <option key={v} value={v}>{label}</option>
                    ))}
                  </select>
                </div>
                <div className="admin-row">
                  <label style={{ alignSelf: "center", fontSize: 13, opacity: 0.8 }}>Depth:</label>
                  <select value={[10, 20, 30].includes(faqDepth) ? faqDepth : "custom"}
                          onChange={(e) => setFaqDepth(e.target.value === "custom" ? 15 : Number(e.target.value))}>
                    <option value={10}>Basic (10)</option>
                    <option value={20}>Thorough (20)</option>
                    <option value={30}>Deep (30)</option>
                    <option value="custom">Custom…</option>
                  </select>
                  {![10, 20, 30].includes(faqDepth) && (
                    <input type="number" min="1" max="60" value={faqDepth} style={{ width: 80 }}
                           onChange={(e) => setFaqDepth(Math.max(1, Math.min(60, Number(e.target.value) || 1)))} />
                  )}
                ...
                <p className="hint" style={{ marginTop: 0 }}>
                  Generation runs in your browser using the API key from Settings — no server key needed.
                  It creates question + answer pairs; review/edit the answers below.
                </p>
              </>
            )}
            {faqSource && faqItems.length === 0 && (
              <p className="hint">No FAQ stored yet — click “Generate FAQ” above.</p>
            )}
            {faqItems.map((it) => (
