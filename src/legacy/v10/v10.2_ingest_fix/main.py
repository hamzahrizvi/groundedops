...
from products import list_products, sources_for
...
class QueryRequest(BaseModel):
    ...
    product: str | None = None  # v2.1 product-scoped chat (key from /products)
...
async def upload(file: UploadFile = File(...),
    ...
    minutes). Poll /upload/status/{job_id} for progress. This eliminates
    the nginx 504 timeout that blocking ingest hit.

    Ingestion provider for doc2query is controlled by INGEST_PROVIDER
    (deepseek/openai/anthropic/local/auto); API keys are read from env in
    the container, so no key needs to travel with the upload."""
    if not APP_STATE["ready"]:
        raise HTTPException(status_code=503, detail="System is still loading")

    content = await file.read()
    filename = file.filename
    # Ingestion uses env-provided keys (container config), not per-request,
    # so large batch ingests can run headless. api_keys left empty -> the
    # provider functions fall back to their env vars.
    job_id = str(uuid.uuid4())
    with _INGEST_LOCK:
        _INGEST_JOBS[job_id] = {"status": "starting", "file": filename,
                                "pct": 0.0, "done": False}
    threading.Thread(target=_ingest_worker,
                     args=(job_id, content, filename, {}), daemon=True).start()
...
@app.get("/products")
def products():
    """Product list for the pre-chat picker (v2.1). Each: {key, name}.
    The customer chooses one before starting a chat; that product scopes
    retrieval for every turn in the conversation."""
...
def query(payload: QueryRequest, x_user_id: str | None = Header(default=None)):
    ...
    # ── Retrieval ────────────────────────────
    t1 = time.time()
    product_sources = sources_for(payload.product)
