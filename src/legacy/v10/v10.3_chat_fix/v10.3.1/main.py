...
from catalog import sources_for
...
def query(payload: QueryRequest, x_user_id: str | None = Header(default=None)):
    ...
    # ── Retrieval ────────────────────────────
    t1 = time.time()
    # v10.3: scope to the chosen PRODUCT, else the CATEGORY (all its docs).
    _scope = payload.product or payload.category
    product_sources = sources_for(_scope, category_key=payload.category)
