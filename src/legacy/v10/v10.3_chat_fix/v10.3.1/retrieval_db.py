...
def _dense_ranking(query: str, collection, limit: int, source_filter: str | None,
    ...
    # Product scoping: Chroma can't substring-match sources server-side and
    # product->sources is many-to-many, so we fetch a LARGE candidate pool
    # and post-filter. v10.2.2: the previous `limit*5` cap could miss a
    # product's chunks entirely for vague queries (they ranked beyond the
    # pool) -> empty dense -> weak score -> false "which product?" clarify
    # even though a product WAS selected. Fetch against the whole
    # collection when scoped; the corpus is small and this runs once/query.
    if product_sources is not None:
        pool = max(collection.count(), limit)
        res = collection.query(query_embeddings=[q_vec.tolist()],
                               n_results=pool, include=["metadatas"])
        if not res.get("ids"):
            return []
