...
_REFERENCE_PATTERNS = [
    # v8.4.3: "tell me more" unanchored — "can you please tell me more
    # about it" starts with a politeness preamble, not the marker.
    ...
    # v10.2.1: "what about X?" is inherently a follow-up (the TOPIC comes
    # from history even when the entity X is explicit). The old pattern
    # only matched "what about that"; production showed "what about the
    # MyCheckr?" slipping through as a standalone query and retrieving
    # nothing useful.
    re.compile(r"^(and |so |ok(ay)? )?(what|how) about\b", re.IGNORECASE),
    re.compile(r"\b(give me that|show me that|what about that|more about (that|it)|more context|more detail)\b", re.IGNORECASE),
    # v8.4.3: pronouns now match ANYWHERE in short queries, not only
    # sentence-initial. "how is it powered" / "where do I plug it in"
    # carry their referent mid-sentence; the old ^-anchored pattern
    # missed them and follow-ups died at retrieval (~0.005 scores).
    # Restricted to short queries (<= 8 words) so a long standalone
    # question that happens to contain "it" is not misclassified.
...
# bare pronouns are strong follow-up evidence in "how is it powered" but
# weak in a 20-word standalone question that mentions "it" incidentally.
_SHORT_ONLY_PATTERNS = {6}  # index of the anywhere-pronoun pattern above
