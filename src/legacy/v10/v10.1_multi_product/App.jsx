...
export default function App() {
  ...
  useEffect(() => {
    ...
    setChats((prev) => {
      ...
      const entry = { id: sessionId, title, messages, updated: Date.now(), product, productName };
  ...
  const runQuery = async (q, forceProvider, forceModel) => {
    ...
    setThinking(true);
    try {
      const data = await api.query({ q, sessionId, forceProvider, forceModel, product });
  ...
  const newChat = async () => {
    // v2.1: choose a product before the chat starts (requested flow).
    setPickProduct(true);
    setView("chat");
  };

  const startChatWithProduct = (p) => {
    setProduct(p ? p.key : null);
    setProductName(p ? p.name : null);
    setSessionId(uuid());
    setMessages([]);
    setSelected(null);
    setIntroDone(true);
  ...
  return (
    <div className="app">
        ...
        <div className="stage" ref={stageRef}>
          {showSplash ? (
            ...
            <div className="stage-inner">
              ...
              {productName && view === "chat" && (
                <div className="product-badge" title="This chat is scoped to one product">
                  {productName}
                </div>
              )}
      ...
      {pickProduct && (
        <div className="overlay" onClick={() => setPickProduct(false)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            <div className="dialog-head"><h2>Which product is this about?</h2></div>
            <p className="hint" style={{ marginTop: 0 }}>
              Choose the product so answers come only from its documentation.
            </p>
            <div className="product-grid">
              {products.map((p) => (
                <button key={p.key} className="product-btn"
                        onClick={() => startChatWithProduct(p)}>
                  {p.name}
                </button>
              ))}
              <button className="product-btn product-all"
                      onClick={() => startChatWithProduct(null)}>
                All products
              </button>
            </div>
          </div>
