...
class FaqEdit(BaseModel):
    # Both optional: the console PATCHes question and answer together, but
    # older callers sent only an answer and must keep working.
...
def faq_edit(faq_id: str, payload: FaqEdit, x_admin_password: str | None = Header(default=None)):
    ...
    # Editing an answer into existence can close an open gap — resolve any
    # unanswered question matching this one so the admin doesn't have to
    # tick it off in two places.
    if (updated.get("answer") or "").strip():
        gaps_store.resolve_matching(updated["question"], updated["id"])
    return updated
...
@app.post("/faq")
def faq_add(payload: FaqAddReq, x_admin_password: str | None = Header(default=None)):
    """v13.0: write a curated answer by hand.

    The console has offered "Write one yourself" since v12 with nothing
    behind it. Hand-written entries are stored edited=True, which is what
    keeps re-ingest and re-drafting from overwriting them.
    """
    _require_admin(x_admin_password)
    try:
        entry = faq_store.add_entry(
            payload.question, payload.answer,
            products=payload.product, category=payload.category)
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
    if (entry.get("answer") or "").strip():
        gaps_store.resolve_matching(entry["question"], entry["id"])
    return entry
...
@app.delete("/faq")
def faq_delete_scope(confirm: bool = False, product: str | None = None,
                     x_admin_password: str | None = Header(default=None)):
    """Bulk-delete every answer for a product, or all of them (v13.0).

    Requires ?confirm=true on top of the admin password. This deletes
    hand-written answers that exist nowhere else and cannot be
    regenerated from a document, so an accidental call — a mistyped URL,
    a client retrying a DELETE — must not be enough to trigger it.
    """
    _require_admin(x_admin_password)
    if not confirm:
        raise HTTPException(
            status_code=400,
            detail="Refusing to bulk-delete without confirm=true. This removes "
                   "hand-written answers permanently.")
    removed = faq_store.delete_scope(product)
    return {"deleted": removed, "scope": product or "all"}


class FaqAutoGenReq(BaseModel):
    source: str
    product: str = ""
    category: str = ""
    count: int = 8
...
def _source_text(source: str, max_chunks: int = 12, cap: int = 8000) -> str:
    """Pull real chunk text for a source, breadcrumbs stripped."""
    from db import get_collection
    col = get_collection()
    got = col.get(where={"source": source}, include=["documents", "metadatas"])
    docs = got.get("documents", []) or []
...
def admin_faq_autogenerate(payload: FaqAutoGenReq,
                           x_admin_password: str | None = Header(default=None)):
    """v13.0: draft Q/A pairs from an ingested document, SERVER-side.

    The existing /faq/generate expects the BROWSER to have called an LLM
    and to POST the finished questions — that works when the operator has
    pasted an API key into the chat UI, but the console is a separate page
    that holds no key, which is why its "Draft answers" button has never
    worked. This route runs the same generation through the app's own
    configured chain (local model, or an API provider if one is set up),
    so drafting works in Free mode with no key anywhere.

    Answers are drafted STRICTLY from the document text and are stored
    un-edited, so the console shows them as needing review. That framing
    matters: these are suggestions to read and fix, not published copy.
    Merging (not replacing) protects any answer already reviewed — see
    faq_store.merge_questions.
    """
    _require_admin(x_admin_password)
    ...
    result = faq_store.merge_questions(
        payload.source, payload.product, pairs, category=payload.category)
    result["source"] = payload.source
    result["model"] = (out or {}).get("model")
    return result
...
def _parse_qa_json(raw: str) -> list[dict]:
    ...
    Small local models wrap JSON in prose or markdown fences perfectly
    routinely, so the array is located within the response rather than the
    whole response being handed to json.loads. Malformed output returns []
    and the caller reports the failure — it is never partially stored.
    """
    import json as _json
    text = (raw or "").strip()
...
# ── Answer gaps (v13.0) — what visitors asked and we could not answer ──

@app.get("/faq/gaps")
def faq_gaps(product: str | None = None, include_resolved: bool = False,
             x_admin_password: str | None = Header(default=None)):
    """The console's upkeep list. Admin-only: these are raw visitor
    questions, which is operational data, not something to serve
    publicly from the widget's own origin."""
    _require_admin(x_admin_password)
    return {"gaps": gaps_store.list_gaps(product, include_resolved),
            "stats": gaps_store.stats()}


@app.post("/faq/gaps/{gap_id}/resolve")
def faq_gap_resolve(gap_id: str, x_admin_password: str | None = Header(default=None)):
    _require_admin(x_admin_password)
    if not gaps_store.resolve(gap_id):
        raise HTTPException(status_code=404, detail="Gap not found")
    return {"resolved": True}


@app.delete("/faq/gaps/{gap_id}")
def faq_gap_delete(gap_id: str, x_admin_password: str | None = Header(default=None)):
    """Dismiss a gap outright — for spam and noise, which a public widget
    will collect. Distinct from resolving: nothing was answered."""
    _require_admin(x_admin_password)
    if not gaps_store.delete(gap_id):
        raise HTTPException(status_code=404, detail="Gap not found")
    return {"deleted": True}


# ── Widget configuration (v13.0) — branding, opening options, forms ────

@app.get("/widget/config")
def widget_get_config():
    """PUBLIC — the embedded widget reads this on load. Admin-only fields
    (notification addresses) are stripped by public_config()."""
    return widget_config.public_config()


@app.get("/admin/widget/config")
def widget_admin_get_config(x_admin_password: str | None = Header(default=None)):
    """Full config including admin-only fields, for the console editor."""
    _require_admin(x_admin_password)
    return widget_config.get()


@app.put("/admin/widget/config")
async def widget_save_config(payload: dict,
                             x_admin_password: str | None = Header(default=None)):
    _require_admin(x_admin_password)
    try:
...
def widget_submit_lead(payload: LeadReq):
    """PUBLIC — a visitor submitting the sales/support form.

    Unauthenticated by necessity (the widget is embedded on a public
    page). Unknown keys in `values` are dropped against the configured
    fields, and the transcript is only kept if that form allows it —
    see widget_config.add_lead.

    NOT RATE LIMITED HERE. This is a public write endpoint and will be
    found by bots; MAX_LEADS caps the file size but does not stop the
    noise. Put this behind the same rate limiting / captcha as any other
    public form before exposing it to the open internet — that belongs at
    the edge (reverse proxy, WAF), not in application code, which is why
    it is called out here rather than half-implemented.
    """
    try:
        ...
        raise HTTPException(status_code=400, detail=str(e))
    # Deliberately does not echo the stored lead back to the public caller.
...
def admin_delete_lead(lead_id: str, x_admin_password: str | None = Header(default=None)):
    _require_admin(x_admin_password)
...
def query(payload: QueryRequest, x_user_id: str | None = Header(default=None)):
    ...
    if confidence == "none":
        ...
        if is_followup:
            ...
            # v13.0: this is a genuine miss — the visitor asked something
            # the corpus does not cover and got the flat rejection. Record
            # it so the console's "People asked, we could not answer" list
            # reflects reality. The clarify branches above are deliberately
            # NOT recorded: asking which product was meant is a working
            # conversation, not a gap (see gaps.py's module docstring).
            gaps_store.record(q, product=payload.product,
                              category=payload.category, reason=reason)
...
Answer:"""
    ...
    if template_leak or generation_failed or flagged:
        ...
        flagged = True
        # v13.0: retrieval found something but we could not stand behind an
        ...
        gaps_store.record(
            q, product=payload.product, category=payload.category,
