...
# v15.2: the React SPA was retired. It was a third chat surface duplicating
# the admin console's test chat and the widget, so every fix had to be made
# three times and the widget - the only one customers see - was always last.
# There is no npm build step any more: the widget is plain browser JS served
# straight from src\widget\ by an explicit route, so editing it takes effect
# on the next request with no build and no stale-bundle trap.
$widgetJs = Join-Path $app "widget\groundedops-widget.js"
if (Test-Path $widgetJs) {
    if (Select-String -Path $widgetJs -Pattern "widget/catalog" -Quiet) {
        Say "  widget: current (uses /widget/* endpoints)" Green
    } else {
        Say "  WARNING: widget still calls the old endpoints." Yellow
    }
    Say "  served from src\widget\ - no build required" DarkGray
} else {
    Say "  widget script MISSING at $widgetJs" Red
}
