...
`/admin` → **Advanced** (root accounts only). Everything here changes how an
answer is produced or checked, and applies to the next question — no restart.

**Checking answers before they are sent.** Every answer is verified against
the pages it came from and held back if it cannot be stood behind. The
first check is an NLI entailment model, which is strict and cannot read a
flattened table — so *Second opinion on held-back answers* lets an LLM read
the page and decide before the answer is discarded. Measured over 57
questions × 3 runs: **68% answered without it, 96% with it**, no regressions,
and slightly faster overall. *Retries before giving up* regenerates a
failed answer; 1 is plenty with the second opinion on.

**Sales questions.** Questions that span the range rather than one product
("which products run on 24V?"). Answer them from the catalogue (every name
and figure copied, never invented), reply with a fixed message of your own,
or treat them as ordinary document questions. To answer from a brochure
instead, upload it on Documents and attach it to a product.

**Reading uploaded documents.** *Drop duplicate scraps of a page* discards
the copy of a sentence that gets stored without its table — it matches the
question but holds none of the answer, and can crowd out the copy that does.
Affects documents uploaded afterwards.
