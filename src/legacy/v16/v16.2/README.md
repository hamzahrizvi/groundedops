...
![The widget answering a question from a product manual](docs/img/widget-demo.gif)

A visitor picks a product range, asks a question in their own words, and gets
an answer with the manual and page it came from. Every answer below is one the
system actually produced against real product manuals — nothing is mocked.
...
The interesting half. An answer that cannot be verified against the retrieved
pages is **not shown** — the system says so and offers a person instead of
guessing:

![A refusal that offers a route to support](docs/img/08-refusal.png)
