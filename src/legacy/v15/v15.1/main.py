...
RETRIEVAL_GATE_THRESHOLD = float(os.getenv("RETRIEVAL_GATE_THRESHOLD", "0.10"))
...
def query(payload: QueryRequest, x_user_id: str | None = Header(default=None)):
    ...
    if confidence == "ambiguous":
        ...
        if _ask_product:
            ...
            try:
                import catalog
                for _c in (catalog.load_catalog().get("categories") or []):
                    for _pr in (_c.get("products") or []):
                        _names[_pr.get("key")] = _pr.get("name") or _pr.get("key")
            except Exception:
                pass
            _labels = [_names.get(k, k) for k in _span]
            clarifying = (
                "That could apply to more than one product, and the answer "
                "differs between them. Which one do you mean? "
                + ", ".join(_labels) + "."
            )
