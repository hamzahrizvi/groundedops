...
def _safe_basename(filename: str) -> str:
    """A bare filename, with any path structure stripped.

    basename() alone already defeats "../../etc/passwd", but it is
    platform-dependent: on POSIX a backslash is a legal filename character, so
    a Windows-style traversal using backslash separators survives it intact.
    A reader has to know that to trust the call site. Normalising both
    separators first makes the guarantee explicit and local, which is also
    what satisfies a taint analyser rather than leaving it to infer
    sanitisation.
    """
    flat = (filename or "").replace("\\", "/")
    return os.path.basename(flat).lstrip(".") or ""


def find(filename: str) -> str | None:
    """Absolute path of a retained original, or None if it is genuinely gone.

    The name may come from a request (see /source_file), so it is reduced to a
    bare basename and the result is confirmed to sit inside a known store
    directory before being handed back.
    """
