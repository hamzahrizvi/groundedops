...
# ╔═══════════════════════════════════════════════════════════════════════╗
# ║  LAYOUT KNOBS — EDIT THESE NUMBERS TO TWEAK THE LOOK / POSITIONS.       ║
# ║  Everything below reads from this dict, so you can play with sizes and  ║
# ║  spacing here in one place without hunting through the CSS.             ║
# ╚═══════════════════════════════════════════════════════════════════════╝
UI = {
    # Sidebar / rail
    "sidebar_width_collapsed": 96,    # px — width of the icon rail when collapsed
    "sidebar_width_expanded": 240,    # px — width when expanded (labels showing)
    "sidebar_top_padding": 2,         # px — space above the first (toggle) icon
    "collapsed_button": 64,           # px — square button size when collapsed
    "collapsed_icon": 34,             # px — icon glyph/image size when collapsed

    # Logos
    "rail_logo_width": 120,           # px — logo width in the expanded rail
    "header_logo_width": 240,         # px — logo width in the top-left header

    # Hover tooltip (collapsed rail only)
    "tooltip_offset": 12,             # px — gap between icon and the hover text
    "tooltip_font": 0.8,              # rem — hover text size

    # Main layout
    "content_max_width": 1300,        # px — max width of the content area
    "panel_chat_ratio": 2,            # chat column weight (details is 1)
}
...
st.markdown(
    ...
    <style>
      ...
      .rail-brand-img img { width:120px; max-width:100%; height:auto; }

      /* Chat input focus indication */
...
# ╔═══════════════════════════════════════════════════════════════════════╗
# ║  UI-KNOBS STYLE OVERRIDES — driven by the UI dict above.                ║
# ║  The hover tooltip is positioned to sit OUTSIDE the sidebar (to its     ║
# ║  right) and only shows when the rail is collapsed (its text content is  ║
# ║  injected only in the collapsed branch — see _collapsed_icon_css).      ║
# ╚═══════════════════════════════════════════════════════════════════════╝
st.markdown(
    f"""
    <style>
      .block-container {{ max-width:{UI['content_max_width']}px !important; }}
      .brand-logo-header img {{ width:{UI['header_logo_width']}px !important; }}
      .rail-brand-img img {{ width:{UI['rail_logo_width']}px !important; }}
      [data-testid="stSidebar"] [data-testid="stSidebarUserContent"] {{
        padding-top:{UI['sidebar_top_padding']}px !important; }}

      /* Let the tooltip escape the sidebar: no ancestor may clip it. */
      [data-testid="stSidebar"], [data-testid="stSidebar"] * {{ overflow:visible !important; }}
      [data-testid="stSidebar"] [class*="st-key-rail_"] {{ position:relative; }}
      [data-testid="stSidebar"] [class*="st-key-rail_"]:hover::after {{
        position:absolute; left:calc(100% + {UI['tooltip_offset']}px); top:50%;
        transform:translateY(-50%); background:var(--ink); color:var(--bg);
        padding:4px 10px; border-radius:8px; white-space:nowrap;
        font-size:{UI['tooltip_font']}rem; z-index:100000; pointer-events:none;
...
# Generic minimal mark in the editorial style (fallback when no logo file).
_LOGO_SVG = """
...
def _collapsed_icon_css() -> str:
    ...
    parts = []
    _sz = UI["collapsed_icon"]
    for key, uri in _custom_icons.items():
        if uri:
            # MASK technique: the PNG/SVG is used only for its SHAPE; the
            # colour comes from background-color (our accent). The icon is
            # always visible in the brand colour no matter what colour the
            # source file happens to be.
            parts.append(
                f".st-key-{key} button {{"
                f" -webkit-mask:url('{uri}') center / {_sz}px {_sz}px no-repeat;"
                f" mask:url('{uri}') center / {_sz}px {_sz}px no-repeat;"
                f" background-color:var(--accent) !important;"
...
def app_settings_body():
    """Unified Settings panel opened from the details-panel gear:
    appearance (light/dark) + DeepSeek key management."""
    st.markdown("**Appearance**")
    _is_dark = st.session_state.get("dark_mode", False)
    if st.button(f"Switch to {'light' if _is_dark else 'dark'} mode",
                 use_container_width=True, key="settings_theme"):
...
if _dialog:
    ...
    def open_app_settings():
        st.session_state.active_panel = "app_settings"
        st.rerun()
...
def rail_btn(label_text: str, material: str, glyph: str, key: str, help_text: str,
    ...
    else a unicode glyph. Shows a text label only when expanded. We do NOT
    pass Streamlit's native help tooltip — expanded mode shows the label,
    and collapsed mode uses our own right-side CSS tooltip. This avoids the
    tooltip popping on top in expanded mode."""
    label = label_text if expanded else "\u200b"  # zero-width when collapsed
    if _BTN_HAS_ICON:
        return st.button(label, icon=material, key=key, use_container_width=expanded)
    prefix = (glyph + "  ") if expanded else glyph
    return st.button(f"{prefix}{label_text if expanded else ''}".strip() or glyph,
                     key=key, use_container_width=expanded)
...
# (Sizes come from the UI knobs at the top of the file.)
_side_w = UI["sidebar_width_expanded"] if rail_expanded else UI["sidebar_width_collapsed"]
if rail_expanded:
    ...
    _sz = UI["collapsed_button"]
    _btn_shape = (f"width:{_sz}px; height:{_sz}px; border-radius:18px; padding:0; margin:10px auto;"
                  "display:flex; align-items:center; justify-content:center; font-size:1.7rem;")

# Force sidebar buttons fully transparent (no white box), both states.
# Collapsed icons use the copper accent (not white); expanded rows use ink.
# IMPORTANT: Streamlit renders the Material icon in its own element with its
# own colour, so we must colour that element directly — setting `color` on
# the button alone leaves the icon invisible.
_icon_color = "var(--ink)" if rail_expanded else "var(--accent)"
_btn_css = f"""
      [data-testid="stSidebar"] .stButton > button,
      [data-testid="stSidebar"] button {{
        background:transparent !important; border:none !important;
        box-shadow:none !important; color:{_icon_color} !important; }}
      [data-testid="stSidebar"] [data-testid="stIconMaterial"],
      [data-testid="stSidebar"] .stButton svg,
      [data-testid="stSidebar"] .stButton p {{
        color:{_icon_color} !important; fill:{_icon_color} !important;
        opacity:1 !important; }}
...
if not _dialog and st.session_state.get("active_panel"):
    ...
    _titles = {"settings": "DeepSeek settings", "app_settings": "Settings", "documents": "Documents"}
    _bodies = {"settings": settings_body, "app_settings": app_settings_body, "documents": documents_body}
    with st.container(border=True):
        top = st.columns([6, 1])
        top[0].subheader(_titles.get(panel, "Settings"))
        if top[1].button("Close", key="panel_close"):
            st.session_state.active_panel = None
            st.rerun()
        _bodies.get(panel, settings_body)()
...
# ── Selectable responses + right-hand details panel ─────────────────────

# Paint a custom settings icon onto the RHS panel's settings button if
# assets/icons/settings.png (or .svg) is provided.
_settings_uri = _icon_data_uri("settings")
if _settings_uri:
    st.markdown(
        f"<style>.st-key-rhs_settings button {{"
        f" -webkit-mask:url('{_settings_uri}') center / 22px 22px no-repeat;"
        f" mask:url('{_settings_uri}') center / 22px 22px no-repeat;"
        f" background-color:var(--accent) !important; }}"
        f".st-key-rhs_settings button p, .st-key-rhs_settings button [data-testid='stIconMaterial']"
        f" {{ opacity:0 !important; }}</style>",
        unsafe_allow_html=True,
    )
...
def render_answer_card(idx: int, meta: dict, content: str) -> None:
    """The answer in the chat column. The card is selectable — selecting it
    opens its sources / model / options in the right-hand panel. The chat
    itself stays clean (no chips); grounded/model live in the panel."""
    selected = st.session_state.get("selected_msg") == idx
    with st.container(border=True):
        st.markdown(content)
        # Clarify pickers stay inline (they're the actionable part).
        if meta.get("role") == "clarify":
            render_clarification_picker(meta, idx)
        label = "Selected — see panel on the right" if selected else "View sources & options →"
        if st.button(label, key=f"sel_{idx}", disabled=selected, use_container_width=True):
            st.session_state.selected_msg = idx
            st.rerun()


def render_rhs_panel() -> None:
    """Right-hand panel: settings + close on top, then the selected
    response's grounded/model chips, sources, rethink, and details."""
    head = st.columns([2, 1, 1], vertical_alignment="center")
    head[0].markdown("### Details")
    with head[1]:
        if _BTN_HAS_ICON:
            _gear = st.button("", icon=":material/settings:", key="rhs_settings", help="Settings")
        else:
            _gear = st.button("⚙", key="rhs_settings", help="Settings")
        if _gear:
            open_app_settings()
    with head[2]:
        if st.button("Close", key="rhs_close"):
            st.session_state.selected_msg = None
            st.rerun()

    idx = st.session_state.get("selected_msg")
    msgs = st.session_state.messages
    meta = msgs[idx]["meta"]
    query = msgs[idx].get("query")
    ...
    if query and meta.get("role") not in ("clarify", "rejected"):
        render_rethink(query, idx, meta)
    render_meta_expander(meta)


# ── Layout: details panel appears ONLY when a response is selected ───────
# (Column ratio comes from UI['panel_chat_ratio'] — edit that to resize.)
_sel = st.session_state.get("selected_msg")
_msgs = st.session_state.messages
_show_panel = (isinstance(_sel, int) and 0 <= _sel < len(_msgs)
               and _msgs[_sel].get("role") == "assistant")

if _show_panel:
    chat_col, panel_col = st.columns([UI["panel_chat_ratio"], 1], gap="large")
else:
...
with chat_col:
    for idx, msg in enumerate(st.session_state.messages):
        if msg["role"] == "assistant" and msg.get("meta"):
            render_answer_card(idx, msg["meta"], msg["content"])
        else:
...
if user_query:
    ...
    if data:
        ...
        # Do NOT auto-open the panel — it appears only when the user clicks
        # a response's "View sources & options" button.
    st.rerun()
