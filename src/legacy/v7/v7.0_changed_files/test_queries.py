...
# DeepSeek key: prefer the DEEPSEEK_API_KEY env var; otherwise fall back
# to the key saved via the GUI (encrypted keyvault on this machine). This
# fixes the "script can't see the key I entered in the GUI" problem — the
# GUI stores the key encrypted in keyvault, not as an env var, so an
# env-only lookup found nothing. Set it explicitly if you prefer:
#   (PowerShell)  $env:DEEPSEEK_API_KEY="sk-..."
# The forced-DeepSeek query below is the only one that requires a key; if
# none is found that single query reports SKIP (not fail).
...
# an answer from DeepSeek" gap. Requires a resolvable DeepSeek key.
FORCED_DEEPSEEK_QUERY = "how do myconnect and mycheckr communicate with each other"

# Queries are grouped by what they exercise. They're phrased the way a
# real installer/user would actually ask — specific and self-contained —
# rather than terse keyword fragments, which both reads better and gives
# retrieval a fair shot at the right chunk.
queries = [
    # ── Factual lookups the corpus should answer cleanly ──────────────
    "what are the default login credentials for the MyConnect app",
    "what is MyCheckr and what is it used for",
    "how does the MyConnect hub communicate with MyCheckr devices",
    "what network connection does the MyConnect hub require",
    "give me an overview of the MyConnect system",

    # ── Structured / checklist extraction ─────────────────────────────
    "what should I confirm before leaving site after installation",
    "how do I connect a tablet to the MyConnect hub",

    # ── Honest 'not in the docs' (correct refusal, NOT a failure) ─────
    "what is the default IP address of the MyConnect hub",

    # ── Under-specified in-domain (should ask which device) ───────────
    "explain why device registration might fail",

    # ── Genuinely out of domain (should be a flat rejection) ──────────
    "what is the capital of france",
    "who won the world cup in 2018",

    # ── Conversational follow-up sequence ─────────────────────────────
    # Step 1: a real, answerable question that sets context.
    "why might MyCheckr registration fail when connecting it to MyConnect",
    # Step 2: an in-context follow-up that names no device on its own —
    # should resolve against Step 1, or ask which device if it can't.
    "which device is that for",
    # Step 3: a reference-marker follow-up whose rewrite still won't
    # retrieve — should be a follow-up clarification, referencing the
    # prior topic, NOT a flat rejection.
    "give me step 1 from that",
    "is there anything else? i have checked the above and they are alright",
    # Step 4: negative control — out-of-domain query late in the session
    # with lots of unrelated history present. Must stay a flat rejection,
    # NOT be swept up as a follow-up just because history exists.
    "what is the capital of france",
]
