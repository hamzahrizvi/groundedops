...
# ── Bug: chat-template boilerplate leak scored as "grounded" ──────────
# Production answer for "introduction of myconnect system": "The
# MyConnect System is a chat between a curious user and an artificial
# intelligence assistant. The assistant gives helpful answers to the
# user's questions." — pure Ollama/phi default system-prompt leakage,
# zero relation to the corpus, yet NLI grounding scored it 0.934
# (grounded) because the sentence makes no concrete claim to
# contradict. is_template_leak catches this deterministically.

def test_is_template_leak_catches_actual_production_leak():
    from text_utils import is_template_leak
    leak = ('The MyConnect System is a chat between a curious user and an '
            'artificial intelligence assistant. The assistant gives helpful '
            "answers to the user's questions.")
    assert is_template_leak(leak) is True


def test_is_template_leak_false_on_real_grounded_answer():
    from text_utils import is_template_leak
    real = 'MyConnect works with MyCheckr through bidirectional REST API communication.'
    assert is_template_leak(real) is False


# ── Bug: vague-but-in-domain standalone queries got a flat rejection ──
# identical to genuinely out-of-domain ones. "explain why device
# registration might fail" / "post installation verification installer
# sign off" / "give me steps to install and verify system is working"
# never specify WHICH device or product, so retrieval misses — but
# they're clearly about something in this domain, unlike "capital of
# France". has_domain_vocabulary lets main.py ask which device instead
# of flatly rejecting these.

def test_has_domain_vocabulary_fires_on_actual_production_misses():
    from text_utils import has_domain_vocabulary
    assert has_domain_vocabulary("explain why device registration might fail") is True
    assert has_domain_vocabulary("post installation verification installer sign off") is True
    assert has_domain_vocabulary("give me steps to install and verify system is working") is True


def test_has_domain_vocabulary_false_on_genuinely_out_of_domain_query():
    from text_utils import has_domain_vocabulary
    assert has_domain_vocabulary("what is the capital of france") is False


# ── Bug: template-leak / total-failure answers were still SHOWN ───────
# to the user. v6.4 correctly FLAGGED the phi chat-template boilerplate
# ("...a chat between a curious user and an artificial intelligence
# assistant...") at grounding 0.000, but main.py still returned the
# boilerplate text as the answer, so the user kept seeing it. And the
# all-fallbacks-failed sentinel ("I was unable to generate a response.")
# was scored 0.993 "grounded" by NLI. main.py now suppresses both:
# after any DeepSeek escalation attempt, an answer that is STILL a
# template leak or a total generation failure is replaced with the
# standard not-found message. These unit tests guard the two detectors
# that drive that suppression (the suppression wiring itself lives in
# main.py's request path and is covered by test_queries.py live).

def test_generation_failure_sentinel_is_detectable():
    # The exact sentinel llm.generate_with_fallback returns when every
    # provider in the chain fails. It must be treated as a non-answer,
    # never run through grounding as if it were real content.
    sentinel = "I was unable to generate a response."
    # It is intentionally NOT a normal refusal phrase (so is_refusal is
    # false); main.py detects it via output model=="none" instead. This
    # test documents that the sentinel text itself is stable so the
    # main.py check (model=="none") and this string stay in sync.
    assert sentinel == "I was unable to generate a response."


def test_template_leak_still_caught_after_hypothetical_escalation():
    from text_utils import is_template_leak
    # Simulates the case where DeepSeek escalation returned yet more
    # boilerplate (or no key was present so nothing changed) — the leak
    # detector must still fire so main.py suppresses it.
    still_leaking = ("This is a conversation between a curious user and an "
                     "artificial intelligence assistant.")
    assert is_template_leak(still_leaking) is True


# ── Feature: disambiguation dropdown options for clarify turns ────────
# Instead of only a free-text clarification, the backend now returns a
# short list of concrete pickable options so the GUI can show a "did you
# mean…?" dropdown. Follow-ups offer recent conversation topics; vague
# in-domain queries offer product/area labels derived from the candidate
# sources retrieval turned up.

def test_clarification_options_followup_offers_recent_topics_newest_first():
    from text_utils import build_clarification_options
    history = [
        {"q": "default login credentials", "a": ".."},
        {"q": "how to connect tablet to hub", "a": ".."},
    ]
    opts = build_clarification_options("followup", history, [])
    # Newest topic first, both present, no dupes.
    assert opts == ["how to connect tablet to hub", "default login credentials"]


def test_clarification_options_in_domain_offers_product_labels_from_sources():
    from text_utils import build_clarification_options
    results = [
        {"source": "MyConnect Environment-v4.pdf"},
        {"source": "MyCheckr User Manual-v6.pdf"},
        {"source": "MyCheckr Mini User Manual-v2.pdf"},
        {"source": "MyConnect Environment-v4.pdf"},  # dup source
    ]
    opts = build_clarification_options("ambiguous_in_domain", [], results)
    assert opts == ["MyConnect", "MyCheckr", "MyCheckr Mini"]


def test_clarification_options_mini_before_plain_mycheckr():
    from text_utils import _product_label_for_source
    # "MyCheckr Mini" must not be mislabelled as plain "MyCheckr".
    assert _product_label_for_source("MyCheckr Mini User Manual-v2.pdf") == "MyCheckr Mini"
    assert _product_label_for_source("MyCheckr User Manual-v6.pdf") == "MyCheckr"


def test_clarification_options_empty_when_nothing_concrete():
    from text_utils import build_clarification_options
    assert build_clarification_options("followup", [], []) == []
    assert build_clarification_options("ambiguous_in_domain", [], []) == []
    # Unknown source -> no label -> no option.
    assert build_clarification_options("ambiguous_in_domain", [], [{"source": "Random.pdf"}]) == []
