...
# ── Styling: warm "editorial" theme (cream + copper), with a dark ───────
# variant of the SAME palette driven by the browser/OS colour scheme.
# Headings use Playfair Display (serif); body stays a clean sans. Buttons
# are thin outlined pills that fill with a soft copper tint on hover.
#
# Note: colours are held in CSS variables and flipped under
# @media (prefers-color-scheme: dark), so the look follows the OS/browser
# scheme regardless of Streamlit's own light/dark setting. For a perfectly
# consistent frame, match Streamlit's theme to your OS scheme.
st.markdown(
    """
    <style>
      ...
      @media (prefers-color-scheme: dark) {
        :root {
          ...
          --ink:#EDE4D7; --muted:#A2968899; --hairline:rgba(201,138,94,0.34);
      ...
      /* Hero */
      .go-hero { display:flex; align-items:center; gap:16px; margin:0 0 2px 0; }
      .go-logo { width:44px; height:44px; flex:0 0 auto; }
      .go-title { font-size:2.1rem; line-height:1.05; }
      .go-sub { color:var(--muted); font-size:0.92rem; margin:2px 0 6px 60px;
...
with st.sidebar:
    ...
    try:
        ...
        if not sources:
            ...
            for src in sources:
                st.caption(src)
                if st.button("Remove", key=f"del_{src}", use_container_width=True):
