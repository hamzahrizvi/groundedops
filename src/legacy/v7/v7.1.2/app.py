...
with st.sidebar:
    ...
    if st.button("🔑", key="rail_settings", help="DeepSeek key & settings"):
        open_settings()
    if st.button("🗂", key="rail_docs", help="Documents — upload & manage"):
        open_documents()

    st.markdown('<hr class="rail-sep"/>', unsafe_allow_html=True)

    if st.button("＋", key="rail_new", help="New conversation"):
        do_new_conversation()
    if st.button("↻", key="rail_reset", help="Reset knowledge base"):
        do_reset_kb()

    # Small status dot showing whether a DeepSeek key is set.
    _dot = "go-dot-on" if keyvault.has_key() else "go-dot-off"
    st.markdown(f'<span class="rail-dot"><span class="go-dot {_dot}"></span></span>',
                unsafe_allow_html=True)

# Inline fallback panel (only used when st.dialog isn't available)
