...
st.markdown(
    ...
    <style>
      ...
      .rail-dot { display:block; text-align:center; margin-top:6px; }
...
# ── Optional custom PNG/SVG icons ────────────────────────────────────────
# Drop files in src/assets/icons/ and they'll be painted onto the matching
# rail buttons (targeted by Streamlit's per-key `.st-key-<key>` class) as
# base64 data-URIs — no static-file server needed. Missing icons fall back
# to Material icons / emoji automatically.
#
# Expected filenames (any of .png/.svg):
#   toggle, key, folder, new, reset, theme_dark, theme_light
import base64 as _b64


def _icon_data_uri(name: str) -> str | None:
    for ext, mime in [("png", "image/png"), ("svg", "image/svg+xml")]:
        p = os.path.join(_BASE_DIR, "assets", "icons", f"{name}.{ext}")
        if os.path.exists(p):
            try:
                with open(p, "rb") as f:
...
# Map each rail button key -> icon file basename.
_RAIL_ICON_NAMES = {
    "rail_toggle": "toggle",
    "rail_settings": "key",
    "rail_docs": "folder",
    "rail_new": "new",
    "rail_reset": "reset",
    "rail_theme_dark": "theme_dark",   # shown when currently light
    "rail_theme_light": "theme_light", # shown when currently dark
}

# Build CSS that paints any available custom icons. When an icon exists we
# hide the button's text/Material glyph and show the image centered.
_custom_icons = {k: _icon_data_uri(v) for k, v in _RAIL_ICON_NAMES.items()}
_HAVE_CUSTOM_ICONS = any(_custom_icons.values())
_icon_css_parts = []
for _key, _uri in _custom_icons.items():
    if not _uri:
        continue
    _icon_css_parts.append(
        f".st-key-{_key} button {{ background-image:url('{_uri}') !important;"
        f" background-repeat:no-repeat; background-position:center;"
        f" background-size:24px 24px; }}"
        f".st-key-{_key} button p, .st-key-{_key} button [data-testid='stIconMaterial'] {{"
        f" opacity:0 !important; }}"
    )
if _icon_css_parts:
    st.markdown("<style>" + "".join(_icon_css_parts) + "</style>", unsafe_allow_html=True)


_LOGO_IMG = next(
    (p for p in ["assets/logo.png", "assets/logo.svg", "logo.png", "logo.svg"]
     if os.path.exists(os.path.join(_BASE_DIR, p))),
    None,
)

if _LOGO_IMG:
    # Their custom logo already includes the wordmark — show it bigger,
    # top-left, and skip the text title + tagline.
    st.image(os.path.join(_BASE_DIR, _LOGO_IMG), width=240)
else:
    st.markdown(
...
# after the base <style>, so it wins). This flips the app surfaces we style
# ourselves; Streamlit's own base theme comes from .streamlit/config.toml.
if dark_mode:
    _palette = """
      :root {
...
# State-dependent sidebar width + button shape. Collapsed = icon-only rail
# (transparent, no white box); expanded = full-width labelled rows.
_side_w = 232 if rail_expanded else 82
_btn_shape = (
    "width:100%; justify-content:flex-start; gap:10px; border-radius:12px;"
    "padding:0.5rem 0.85rem; margin:4px 0; font-size:0.95rem;"
    if rail_expanded else
    "width:52px; height:52px; border-radius:16px; padding:0; margin:6px auto;"
...
st.markdown(
    ...
    f"min-width:{_side_w}px !important;}}{_btn_css}</style>",
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    _toggle_mat = ":material/left_panel_close:" if rail_expanded else ":material/left_panel_open:"
    if rail_btn("Collapse" if rail_expanded else "", _toggle_mat, "«" if rail_expanded else "»",
                "rail_toggle", "Expand / collapse menu", rail_expanded):
        st.session_state.rail_expanded = not rail_expanded
        st.rerun()
    ...
    # Key-status dot at the bottom of the rail.
    _dot = "go-dot-on" if keyvault.has_key() else "go-dot-off"
    st.markdown(f'<span class="rail-dot"><span class="go-dot {_dot}"></span></span>',
                unsafe_allow_html=True)
