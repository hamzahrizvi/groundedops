...
st.markdown(
    ...
    <style>
      ...
      /* Keep Streamlit's own menu accessible (this is the "settings"
         that had gone missing) — just blend the header into the bg. */
      [data-testid="stHeader"], [data-testid="stToolbar"] { background:transparent !important; }
      [data-testid="stDecoration"] { display:none; }   /* top gradient run-bar */
      footer { visibility:hidden; }

      /* App surfaces */
      .stApp, [data-testid="stAppViewContainer"], [data-testid="stMain"] { background:var(--bg); }

      /* Left ICON RAIL (Claude-style): narrow sidebar, icon-only buttons
         that open panels. No wall of text. */
      ...
      [data-testid="stSidebar"] .stButton > button {
        width:52px; height:52px; border-radius:16px; padding:0; margin:7px auto;
      ...
      .rail-sep { border:none; border-top:1px solid var(--hairline); margin:12px 8px; }
      .rail-dot { display:block; text-align:center; margin-top:2px; }
      ...
      .go-sub { color:var(--muted); font-size:0.95rem; margin:4px 0 6px 0;
      ...
      /* The sidebar must not be fully dismissible — we ship our own
         expand/retract toggle instead. Hide Streamlit's collapse control. */
      ...
      .rail-toggle-wrap { display:flex; justify-content:center; margin:2px 0 8px 0; }
    </style>
    """,
...
# State-dependent sidebar width + button shape. Collapsed = circular icon
# rail; expanded = full-width labelled rows (Claude-style).
_side_w = 232 if rail_expanded else 82
if rail_expanded:
    _btn_css = """
      [data-testid="stSidebar"] .stButton > button {
        width:100%; justify-content:flex-start; gap:10px;
        border-radius:12px; padding:0.5rem 0.85rem; margin:4px 0; font-size:0.95rem; }
    """
else:
    _btn_css = """
      [data-testid="stSidebar"] .stButton > button {
    ...
    """
st.markdown(
    f"<style>[data-testid='stSidebar']{{width:{_side_w}px !important;"
