...
One file changed: `main.py` (drop-in, replaces v8.3's). THREE fixes, all
aimed at "refuse unless clearly in-domain" for the public bot.

## Fix 3 — Context relative-score floor (case 01, WEEE pollution)
**What:** Context was a flat `results[:3]`; weak straggler chunks rode into
the prompt behind a strong top hit. Now a chunk enters the context only if it
scores ≥ `CONTEXT_FLOOR_RATIO` (default 0.5, env-overridable) of the top
chunk's score.
**Why:** "what is MyCheckr" retrieved the General Description at ~0.9996, but
a WEEE/waste-disposal compliance chunk limped into the top-3 and the model
stitched both into one answer. Grounding can't catch this — the text IS from
the docs (irrelevant but grounded, scored 0.84).
**CORRECTION to earlier analysis:** the WEEE text lives in the MyCheckr user
manuals themselves (both full and Mini), NOT the ICU doc — so Fix 2's source
exclusion does NOT address case 01. This floor is the fix for it. Ratio is
relative, not absolute, so multi-chunk answers with close scores keep all
their chunks, and borderline queries keep their support.
