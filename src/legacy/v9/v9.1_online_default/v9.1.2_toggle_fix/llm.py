...
_API_ONLY_CHAIN: list[tuple[str, str]] = [("deepseek", "deepseek-chat")]
...
def _chain_for(role: str) -> list[tuple[str, str]]:
    ...
    from runtime_config import get_generation_mode, get_online_provider
    if get_generation_mode() == "api":
        # v9.1.1: Online provider is user-selectable (Settings): deepseek
        # (default), openai, or anthropic. Default models are
        # env-overridable (ONLINE_OPENAI_MODEL / ONLINE_ANTHROPIC_MODEL /
        # ONLINE_DEEPSEEK_MODEL).
...
def condense_query(current_query: str, history: list[dict], model: str = CONDENSE_MODEL) -> str:
    ...
    # GENERATION_MODE=api (v8.5): condensation runs on DeepSeek so the
    # whole answering path is Ollama-free (website/production config).
