...
import { SendIcon, DollarIcon } from "./icons.jsx";
...
export default function App() {
  ...
  const newChat = async () => {
    // v9.1.1: previous chat is already persisted — just start fresh.
    setSessionId(uuid());
    setMessages([]);
    setSelected(null);
    setIntroDone(true);
    ...
    <div className="app">
        ...
        <div className="stage" ref={stageRef}>
          {showSplash ? (
            ...
            <div className="stage-inner">
              {/* v9.1.1: single mode toggle — $ icon (lit when online) +
                  Online/Offline label, matching the sidebar icon style.
                  Click opens the ModeDialog for the opposite mode. */}
              <button
                ...
                onClick={() => setModeDialog(mode === "api" ? "local" : "api")}
              >
                <DollarIcon className="mode-ico" />
                <span>{mode === "api" ? "Online" : "Offline"}</span>
              </button>
              {/* v9.1.1: chats panel — right edge */}
              <button className="chats-tab" onClick={() => setChatsOpen((v) => !v)}>
              ...
              {chatsOpen && (
                <div className="chats-panel">
                  <div className="chats-head">
                    <b>Chats</b>
                    <button className="btn" onClick={() => { newChat(); setChatsOpen(false); }}>New</button>
                  </div>
                  {chats.length === 0 && <p className="hint">No saved chats yet.</p>}
                  {chats.map((c) => (
                    <div key={c.id} className={`chat-row ${c.id === sessionId ? "current" : ""}`}>
                      <button className="chat-title" onClick={() => openChat(c)}>{c.title}</button>
      ...
      {startupKeys && (
        <StartupKeysDialog
          ...
          onSwitchFree={() => { setStartupKeys(false); setModeDialog("local"); }}
        />
      )}
