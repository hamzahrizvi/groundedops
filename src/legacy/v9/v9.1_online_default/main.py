...
from runtime_config import get_settings, set_generation_mode, set_local_models_loaded
...
def models_warmup():
    """Manually load phi + mistral into Ollama (v8.6). Startup no longer
    does this automatically — local models cost RAM and load time, and in
    api mode they're unused. The UI settings panel calls this with a
    disclaimer that free/local mode takes time and memory."""
    results = warmup_local_models(["phi", "mistral"])
...
def models_unload():
    """Unload phi + mistral from Ollama memory (v8.6) — offered by the UI
    when the user switches to online mode, so local models stop holding
    RAM. Ollama unloads a model when sent keep_alive=0."""
    import requests as _requests
    from llm import OLLAMA_URL
    results = {}
    for model in ("phi", "mistral"):
        try:
