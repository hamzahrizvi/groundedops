...
# GENERATION_MODE (v8.5): "local" (default) answers with Ollama models per
# the role map below; "api" routes ALL answering to DeepSeek — the
# production/website configuration (2-8s latency instead of 30-240s CPU
# generation). Condensation also switches to DeepSeek in api mode (see
# llm.condense_query), so Ollama can be fully offline. Embeddings,
# reranker, and NLI grounding are in-process sentence-transformer models,
# not Ollama — they run locally in BOTH modes.
# v8.6: mode now lives in runtime_config (UI-switchable at runtime);
# the env var still sets the initial value there.
from runtime_config import get_generation_mode

# Primary (provider, model) per role
MODEL_MAP: dict[str, tuple[str, str]] = {
    "extract":   ("local", "mistral"),
    # ROUTING FIX (v8.4.2): "fast" no longer answers with phi. Observed in
    # production: "what is a mycheckr" classified as fast -> phi produced
    # incoherent/blended answers (WEEE boilerplate stitched into product
    # definitions), and because the fallback chain only fires on FAILURE,
    # not on bad answers, mistral never got a chance. Phi remains the
    # condensation model (llm.condense_query); it is not an answering
    # model. The latency win never justified the quality cliff on the
    # most common question type a support bot receives.
    "fast":      ("local", "mistral"),
...
def route_model(query: str) -> tuple[str, tuple[str, str]]:
    ...
    # API mode: role classification still runs (it drives structured
