...
def _generate_questions(chunk: str, n: int = 4) -> list[str]:
    """DOC2QUERY (v8.5 / Phase 1): generate likely user questions for a
    chunk at ingest time. Each question is stored as its own embedded
    entry pointing back to the parent chunk, so retrieval can match
    user-question-to-generated-question — directly attacking the
    phrasing-mismatch failure class ("sign in" vs "login", terse or
    misspelled queries).

    Provider: DeepSeek by default (offline curation — pennies, minutes);
    falls back to local mistral if no key; DOC2QUERY=off disables
    entirely (ingestion then behaves exactly as v8.4.x)."""
    ...
    try:
        from llm import _call_deepseek, _call_ollama
        result = _call_deepseek(prompt, timeout=30)
...
def ingest_file(content: bytes, filename: str) -> int:
    ...
    try:
        ...
        collection.add(
            ...
            metadatas=[{"source": filename, "kind": "chunk"} for _ in texts],
            ids=ids,
        )
        ...
        for i, chunk in enumerate(texts):
            for j, question in enumerate(_generate_questions(chunk)):
