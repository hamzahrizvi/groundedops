...
WHAT AN ARCHIVE IS
------------------
A plain zip with a `manifest.json` at the root. Plain on purpose: an
operator can open it, see what is in it, and pull one file out without this
code. A backup format you need the application to read is a backup format
that fails you on the day the application will not start.
...
def create_archive(created_by: str = "", level: str = "support",
                   ...
                   include_index: bool = True) -> tuple[bytes, dict]:
    ...
    logger.info(f"backup exported by {created_by or 'unknown'} ({level}): {counts}")
...
def suggested_filename(manifest: dict) -> str:
    ...
    return f"groundedops-{kind}-backup-{stamp or 'export'}.zip"
