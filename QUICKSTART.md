...
```html
<script
  src="http://localhost:8081/groundedops-widget.js"
...
The bundle is served by the dedicated `widget` container on port `8081`,
so its caching and uptime are independent of the app frontend — point a
CDN there in production. The identical file is also reachable at
`http://localhost:8080/widget/groundedops-widget.js` from the frontend
image, which is handy for local testing.
