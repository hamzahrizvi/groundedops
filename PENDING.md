...
- **Markdown is rendered in the admin console's test chat only.** The React
  app (`src/frontend/src/`) and the public widget still show list/table
  markup as literal text. This is the surface customers actually see.
  `renderMarkdown` in `admin.html` (~60 lines, no dependency) is the reference
  implementation to port — it escapes first, then re-introduces only bullets,
  numbered lists, tables, bold, and code.

- **`offer_support` / `product_options` / `download_url` / `page_label` are
  wired in the backend and the admin test chat only.** Same gap as above:
  the widget and React app don't render any of the v15 response fields yet.
