...
│   ├── .obsidian/
│   ├── 10 Backend/               one .md per src/*.py, GENERATED by tools/build_vault.py
│   │                             (import graph, endpoints, functions) + hand-written Tooling.md
│   ├── 20 Console and Widget/    admin.html, the widget JS and the WordPress plugin
│   ├── 30 Infrastructure/        the three compose services + the Windows install
│   ├── 40 Concepts/              hand-written: pipeline, answerability, grounding, access…
│   └── 50 API/                   GENERATED: one note per router + API Surface hub.
│                                 `python tools/build_vault.py` refreshes the generated
│                                 notes and fails on any dangling wikilink.
├── .github/
│   └── workflows/
